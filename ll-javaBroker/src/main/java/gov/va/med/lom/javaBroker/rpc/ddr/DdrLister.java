...
public class DdrLister extends DdrQuery {
  ...
  public DdrLister(RpcBroker rpcBroker) {
    super(rpcBroker);
  }
  ...
  public List<String> execute() throws Exception {
    ...
    if ((from != null) && (!from.equals(""))) {
      mult.setMultiple("\"FROM\"", from);
    }
    if ((part != null) && (!part.equals(""))) {
      mult.setMultiple("\"PART\"", part);
    }
    ...
    if ((moreFrom != null) && (!moreFrom.equals(""))) {
      mult.setMultiple("\"FROM\"", moreFrom);
    }
    if ((moreIens != null) && (!moreIens.equals(""))) {
      mult.setMultiple("\"IENS\"", moreIens);
    }    
    params.addMult(mult); 
  ...
  public static DdrLister buildIenNameQuery(RpcBroker rpcBroker, String fileNumber) {
  ...
  public static DdrLister buildFileQuery(RpcBroker rpcBroker, String fileNumber, String fieldString) {
    DdrLister query = new DdrLister(rpcBroker);
    query.setFile(fileNumber);
