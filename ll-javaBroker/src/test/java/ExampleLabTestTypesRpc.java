/*
 * ExampleLabTestTypesRpc.java
 * 
 * Author: Rob Durkin (rob.durkin@med.va.gov)
 * Version 1.3 (07/05/2005)
 *  
 * Prints a list of lab test types.
 *
 * Usage: java ExampleLabTestTypesRpc AUTH_PROPS
...
public class ExampleLabTestTypesRpc {
  ...
  public static void main(String[] args) {
    ...
    String verify = null;
    if (args.length != 1) {
      System.out.println("Usage: java ExampleLabTestTypesRpc AUTH_PROPS");
    ...
    try {
      ...
      if (vistaSignonResult.getSignonSucceeded()) {
        ...
        LabTestTypesRpc labTestTypesRpc = new LabTestTypesRpc(rpcBroker);
        // get and print the list of lab report types
         LabReportList labReportList = labTestTypesRpc.getLabReportLists();
        LabReport[] labReports = labReportList.getLabReports();
        System.out.println("---------- LAB REPORTS LISTS ------------");
        for(int i = 0; i < labReports.length; i++) {
          System.out.println(labReports[i].getName() + " (IEN: " + labReports[i].getIen() + ", RPC: " + labReports[i].getRpc() + ")");
        }
      } else {
